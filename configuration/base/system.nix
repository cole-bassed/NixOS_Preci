{
  ...
  #  `cfg = config.${top}.${dom}.${mod}`, so a field set directly on
  #  `${top}.base.system.*` (with no `host.nix` entry at all) behaves
  ...
  opts = {
    ...
    #  direct `${top}.base.system.stateVersion` assignment exists
        ...
        assertions = [
          {
            ...
            message = "${top}.${dom}.${mod}.interface.bootLoader must be one of \"systemd-boot\", \"grub\", \"grub-efi\"; got \"${cfg.interface.bootLoader}\" for host \"${cfg.name}\".";
          }
        ];
